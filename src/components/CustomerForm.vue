  ...
  <div class="container mx-auto my-5">
    ...
    <FormBlock header="Order Summary">
      <table class="table table-borderless table-responsive">
        <tbody>
          <tr>
            <td>{{ monthly.standart }} Standart/mo.</td>
            <td>{{starting_date}}</td>
            <td>{{ resultTableData.standartAmoutn }}</td>
          </tr>
          <tr>
          ...
          <tr v-for="(coll, index) in resultTableData.calls" :key="coll.id" class="text-muted" style="font-size: 14px">
            <td class="ps-4">{{index+1}}. {{ coll.label }}</td>
            <td>{{coll.start}} </td>
            <td>{{ coll.amount }}<br class="d-block d-md-none"> ({{coll.count}} x {{ coll.price }})</td>
          </tr>
          <tr>
...
export default {
  ...
  computed: {
    resultTableData() {
      ...
      const standartCallsAmount = standartPrice * this.monthly.standart;
      ...
      const total = customCallsAmount + standartCallsAmount;
      const discount =
        total -
      ...
      const calls = this.calls.filter(notEmpty).map((c) => {
        const callItem = this.callTypes.find((call) => !c?.type || call.id === c.type);
        return {
          label: callItem?.name,
          count: c.count,
          start: c.start_month,
      ...
      return {
        custom: null,
        standart: null,
        standartAmoutn: moneyFilter(standartCallsAmount),
